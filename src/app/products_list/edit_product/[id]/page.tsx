...
export default function CurrentPage({ params }) {
  ...
  useEffect(() => {
    ...
    axios.get(apiUrl).then(async (response) => {
      ...
      data.productCategory = referenceTypeList.filter(
        ({ text }) => text == data.category
      )[0].id;
      data.cutoffsForProduct = data.cutoffsForProduct.map((el) => {
        el.value = JSON.parse(el.value);
        return el;
      });
      data.tablesForParam = data.tablesForParam.map((el) => {
        el.value = JSON.parse(el.value);
        return el;
      });
  ...
  return (
    <Page>
      <Spacer space={20} />
      <Text className="title">Редактирование продукта</Text>
      <Spacer space={20} />
